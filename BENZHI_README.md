...
这是一个基于 Go 实现的后端服务，用于承载 distant-water-fishing-ops-202608-root 的业务处理、数据管理与稳定运行。
...
./build_benzhi_docker.sh benzhi-task-143-amd64 linux/amd64
./build_benzhi_docker.sh benzhi-task-143-arm64 linux/arm64
docker run -it benzhi-task-143-amd64:latest
docker run -it --platform linux/arm64 benzhi-task-143-arm64:latest
```

## 题目验证命令

1. 预期退出码 1：`go test ./internal/runtimeops -run '^TestTask0023CheckpointReadsCannotCrossFleetBoundary$' -count=1`
