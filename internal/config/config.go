...
type Config struct {
	...
	CatchLandingTaskTTL time.Duration
...
func Load() (Config, error) {
	cfg := Config{
		...
		DatabasePath:     envOr("DATABASE_PATH", "./data/fishingops.db"),
		BusinessTimezone: envOr("BUSINESS_TIMEZONE", "Asia/Shanghai"),
		LogLevel:         envOr("LOG_LEVEL", "info"),
	}
	var err error
	if cfg.CatchLandingTaskTTL, err = durationEnv("HANDOFF_TTL", 30*time.Minute); err != nil {
		return Config{}, err
	}
	...
	if cfg.WorkerBatchSize, err = intEnv("WORKER_BATCH_SIZE", 50); err != nil {
		return Config{}, err
	}
...
func (c Config) Validate() error {
	...
	if c.CatchLandingTaskTTL < time.Minute || c.CatchLandingTaskTTL > 24*time.Hour {
		problems = append(problems, errors.New("HANDOFF_TTL must be between one minute and one day"))
	}
