...
func (s *Store) RecordEvent(ctx context.Context, event Event) error {
	...
	if _, err := s.db.ExecContext(ctx, `INSERT INTO events(id,tenant_id,batch_id,status,magnitude) VALUES(?,?,?,?,?)`, event.ID, event.TenantID, event.BatchID, event.Status, event.Magnitude); err != nil {
		return err
	}
