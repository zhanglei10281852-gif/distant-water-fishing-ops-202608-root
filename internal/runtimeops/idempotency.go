...
func (s *Store) SaveCommand(ctx context.Context, c Command) error {
	return withTx(ctx, s.db, func(tx *sql.Tx) error {
		rows, err := tx.QueryContext(ctx, `SELECT method,path,request_hash,response FROM commands WHERE tenant_id=? AND key=? ORDER BY method,path LIMIT 1`, c.TenantID, c.Key)
		if err != nil {
			return err
		}
		if rows.Next() {
			var method, path, hash string
			var response []byte
			if err = rows.Scan(&method, &path, &hash, &response); err != nil {
				rows.Close()
				return err
			}
