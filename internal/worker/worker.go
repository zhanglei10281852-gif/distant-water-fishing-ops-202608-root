...
func (w *Worker) RunOnce(ctx context.Context) error {
	...
	return w.store.WithTx(ctx, func(tx repository.Tx) error {
		expired, err := tx.ExpireCatchLandingTasks(ctx, now, w.batch)
		if err != nil {
			return err
		}
		if len(expired) > 0 {
			w.logger.Info("expired handoff tasks", "count", len(expired))
		}
...
func (w *Worker) processJob(ctx context.Context, tx repository.Tx, job domain.OutboxJob) error {
	...
	switch job.Kind {
	case "fishing_voyage_planned", "catch_anomaly_review":
		var marker string
		if len(job.Payload) > 0 {
			if err := json.Unmarshal(job.Payload, &marker); err != nil && job.Kind == "catch_anomaly_review" {
				return fmt.Errorf("decode catch_anomaly job: %w", err)
			}
		}
		return nil
	default:
